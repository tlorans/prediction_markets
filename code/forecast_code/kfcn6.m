...
kernel2   =exp(-2*sin(delta_t/alfa(3)).^2/alfa(4)^2); %main time
kernel3   =exp( -delta_t .^2/2/alfa(5)^2 );
kernel4   =exp(-2*sin(delta_t/alfa(6)).^2/alfa(7)^2); %main time
kernel5   =exp( -delta_t .^2/2/alfa(8)^2 );
...
% size(kernel1_v1);
% size(kernel_t2);
y=alfa(9)*kernel1+alfa(10)*kernel2.*kernel3+alfa(11)*kernel4.*kernel5 ;
% y=kernel1;
